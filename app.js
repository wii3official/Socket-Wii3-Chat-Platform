...
app.get('/api/online', (req, res) => {
  res.status(200).send('Wii3 Chats is online!')
})
...
io.on('connection', (socket) => {
	socket.on('join', (data) => {
		...
		if (!Object.keys(allsockets).includes(room)) {
			...
			socket.emit('message', {
				name: 'server',
				message: `Welcome to Wii3 Rooms v2! You are currently in room "${socket.proto.room}". To join a different room, type /join [room]. Type /help to see more commands`
			});
...
function randHex(len) {
	var letters = '0123456789abcdefghi';
	var color = '';
	for (var i = 0; i < len; i++) {
